...
USER_NAME = os.environ['USER_NAME'] # e.g. 'Feroan101'
try:
    # Birth date (YYYY-MM-DD) used for the age stat
    BIRTHDAY = datetime.datetime.strptime(os.environ['BIRTHDAY'], '%Y-%m-%d')
except KeyError:
    raise SystemExit(
        'The BIRTHDAY environment variable (YYYY-MM-DD) is required for the age stat. '
        'Set it in your repository variables or locally before running.'
    ) from None
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=[]):
    ...
    current_edges = [edge for edge in request.json()['data']['user']['repositories']['edges'] if edge is not None and edge.get('node') is not None]
    if request.json()['data']['user']['repositories']['pageInfo']['hasNextPage']:   # If repository data has another page
        edges += current_edges                                          # Add on to the LoC count
